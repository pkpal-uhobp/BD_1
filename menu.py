...
class MainWindow(QMainWindow):
    ...
    #     update_widgets = {}
    #
    #     def serialize_authors_field(value):
    #         """Превращает значение авторов из БД (список, строка, с/без скобок) в строку для QLineEdit"""
    #         if isinstance(value, list):
    #             return ", ".join(str(item).strip() for item in value)
    #         elif isinstance(value, str):
    #             text_value = value.strip('[]')
    #             return ", ".join([x.strip("'\" ") for x in text_value.split(",") if x.strip()])
    #         return ""
    #
    #     def deserialize_authors_field(text):
    #         """Преобразует строку авторов (через запятую) в массив для записи в БД"""
    ...
    def on_header_clicked(self, logical_index: int):
        """
        Обрабатывает клик по заголовку столбца: сортирует по возрастанию/убыванию.
        Учитывает специфику колонки 'authors' (массив).
        """
        if not self.current_table_data:
            return

        header = self.data_table.horizontalHeaderItem(logical_index)
        if not header:
            return
        ...
        try:
            ...
            def sort_key(row):
                ...
                # Специальная обработка для колонки 'authors'
                if column_name == "authors":
                    if isinstance(value, str):
                        # Если уже строка — берем первый автора до запятой или всю строку
                        first_author = value.split(",")[0].strip() if "," in value else value.strip()
                        return first_author.lower()
                    elif isinstance(value, list):
                        # Если список — берем первого автора
                        if len(value) > 0:
                ...
                # Для чисел — пробуем преобразовать
                if isinstance(value, str):
                    if value.isdigit():
                        return int(value)
                    try:
                ...
                # По умолчанию — строка в нижнем регистре
                return str(value).lower()

            sorted_data = sorted(self.current_table_data, key=sort_key, reverse=reverse)

            # Перезаполняем таблицу
            self._display_data_in_table(sorted_data)
